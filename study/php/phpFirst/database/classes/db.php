...
class Database{
    ...
    /*
     * 将php的结果用console.log输出
     * @param array | string  $val 需要输出的数据
     * */
    function log($val){
    ...
    /*
     * 原本是和log合并在一起的，用来显示传入参数在引用文件中的行数，但我觉得还是拆分开来好
     * @param array | string  $val 需要输出数据所在行数的数据
     * */
    function line($val){
        $debug = debug_backtrace();
        unset($debug[0]['args']);
